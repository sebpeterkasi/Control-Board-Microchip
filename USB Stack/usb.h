...
typedef struct sUSBBufferTX
{
	unsigned char data[CONF_USB_TX_BUFFER];
...
char usbTransmitByte(unsigned char);
void usbTransmitBufferContent(void);
void usbResetBuffer(void);
char usbComService(void);
